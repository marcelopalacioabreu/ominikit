...
test "BCE loss backward CPU and CPUSIMD (temp full)" {
    ...
    const b = try ft.fromArray(&[_]usize{4}, &[_]f64{ 1.0, 0.0, 1.0, 0.0 });
    defer b.destroy(&alloc);
    const out = try FabricaFuncoesPerda.BCE(&ctx, &alloc, a, b);
    defer out.destroy(&alloc);
    const grad = [_]f64{1.0};
    out.backward(&alloc, &grad);
    var any_nonzero: bool = false;
    ...
    var ctx2 = computacao.ComputacaoCPUSIMDContexto();
    var ft2 = FabricaTensor.init(&ctx2, &alloc);
    const a2 = try ft2.fromArray(&[_]usize{4}, &[_]f64{ 0.9, 0.1, 0.8, 0.2 });
    defer a2.destroy(&alloc);
    const b2 = try ft2.fromArray(&[_]usize{4}, &[_]f64{ 1.0, 0.0, 1.0, 0.0 });
    defer b2.destroy(&alloc);
    const out2 = try FabricaFuncoesPerda.BCE(&ctx2, &alloc, a2, b2);
    defer out2.destroy(&alloc);
    out2.backward(&alloc, &grad);
    var any_nonzero2: bool = false;
    for (0..a2.size) |i| {
...
test "SmoothL1 loss backward CPU and CPUSIMD (temp full)" {
    ...
    const b = try ft.fromArray(&[_]usize{4}, &[_]f64{ 0.0, 0.0, 0.0, 0.0 });
    defer b.destroy(&alloc);
    const out = try FabricaFuncoesPerda.SmoothL1(&ctx, &alloc, a, b);
    defer out.destroy(&alloc);
    const grad = [_]f64{1.0};
    out.backward(&alloc, &grad);
    var any_nonzero: bool = false;
    ...
    const b2 = try ft2.fromArray(&[_]usize{4}, &[_]f64{ 0.0, 0.0, 0.0, 0.0 });
    defer b2.destroy(&alloc);
    const out2 = try FabricaFuncoesPerda.SmoothL1(&ctx2, &alloc, a2, b2);
    defer out2.destroy(&alloc);
    out2.backward(&alloc, &grad);
    var any_nonzero2: bool = false;
    for (0..a2.size) |i| {
